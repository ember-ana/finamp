...
class AndroidAutoHelper {
  ...
  Future<List<BaseItemDto>> getBaseItems(MediaItemId itemId) async {
    ...
    final parentItem = itemId.parentType == MediaItemParentType.collection
        ? BaseItemDto(id: itemId.itemId!, type: itemId.contentType.itemType.baseItemKind)
    ...
    final items = await _jellyfinApiHelper.getItems(
      ...
      includeItemTypes: includeItemTypes.baseItemKind?.jellyfinName,
  ...
  Future<void> playFromSearch(AndroidAutoSearchQuery searchQuery) async {
      ...
      try {
        ...
        if (FinampSettingsHelper.finampSettings.isOffline) {
          ...
          searchResult = await jellyfinApiHelper.getItems(
            ...
            includeItemTypes: TabContentType.playlists.itemType.baseItemKind?.jellyfinName,
        ...
        if (searchResult?.isNotEmpty ?? false) {
          ...
          if (FinampSettingsHelper.finampSettings.isOffline) {
            ...
            items = await _jellyfinApiHelper.getItems(
              parentItem: playlist,
              includeItemTypes: TabContentType.tracks.itemType.baseItemKind?.jellyfinName,
    ...
    try {
      ...
      final selectedResult = searchResult.firstWhere((element) {
        if (itemType == TabContentType.tracks.itemType && searchQuery.extras?["android.intent.extra.artist"] != null) {
          return element.albumArtists?.any(
              ...
              false;
        } else if (itemType == TabContentType.tracks.itemType &&
            searchQuery.extras?["android.intent.extra.artist"] != null) {
          return element.albumArtists?.any(
                (artist) =>
                    (artist.name?.isNotEmpty ?? false) &&
  ...
  Future<List<BaseItemDto>?> _getResults({
    ...
    if (FinampSettingsHelper.finampSettings.isOffline) {
      ...
      if (itemTypes.first == TabContentType.tracks.itemType) {
        // If we're on the tracks tab, just get all of the downloaded items
      ...
      if (itemTypes.first == BaseItemDtoType.artist) {
        ...
        searchResult = await jellyfinApiHelper.getItems(
          ...
          includeItemTypes: itemTypes.map((type) => type.baseItemKind).join(","),
          searchTerm: searchTerm,
          startIndex: 0,
  ...
  bool _isPlayable({BaseItemDto? item, TabContentType? contentType}) {
    final tabContentType = TabContentType.fromItemType(
      item?.type ?? contentType?.itemType.baseItemKind ?? BaseItemKind.audio,
    );
